/******************************************************************************
  ...
  ecat7h.c   (c) 2008,2012 by Turku PET Centre

  Procedures for editing ECAT 7.x header contents.

  Version:
  2008-07-24 Vesa Oikonen
    Content functions copied from previous programs.
  2012-05-09 VO
...
int ecat7EditMHeader(
  ...
  if(strcmp(field, "magic_number")==0) {
    strncpy(h->magic_number, value, 14);
  } else if(strcmp(field, "original_file_name")==0) {
    strncpy(h->original_file_name, value, 32);
    ...
    strncpy(h->serial_number, value, 10);
  } else if(strcmp(field, "scan_start_time")==0) {
    timet=time(NULL); st=localtime(&timet);
    if(sscanf(value, "%d-%d-%d %d:%d:%d", &yy, &mm, &dd, &ho, &mi, &se)!=6)
      return(2);
    st->tm_mday=dd; st->tm_mon=mm-1; st->tm_year=yy-1900;
    st->tm_hour=ho; st->tm_min=mi; st->tm_sec=se; st->tm_isdst=-1;
    h->scan_start_time=mktime(st);
  } else if(strcmp(field, "isotope_name")==0) {
    strncpy(h->isotope_name, value, 8);
    ...
    strncpy(h->radiopharmaceutical, value, 32);
    ...
    strncpy(h->study_type, value, 12);
  } else if(strcmp(field, "patient_id")==0) {
    strncpy(h->patient_id, value, 16);
  } else if(strcmp(field, "patient_name")==0) {
    strncpy(h->patient_name, value, 32);
    ...
    st->tm_mday=dd; st->tm_mon=mm-1; st->tm_year=yy-1900;
    st->tm_hour=0; st->tm_min=0; st->tm_sec=0; st->tm_isdst=-1;
    h->patient_birth_date=mktime(st);
  } else if(strcmp(field, "physician_name")==0) {
    strncpy(h->physician_name, value, 32);
  } else if(strcmp(field, "operator_name")==0) {
    strncpy(h->operator_name, value, 32);
  } else if(strcmp(field, "study_description")==0) {
    strncpy(h->study_description, value, 32);
    ...
    strncpy(h->facility_name, value, 20);
    ...
    strncpy(h->user_process_code, value, 10);
...
int ecat7EditVHeader(
  ...
  if(strcasecmp(field, "DATA_TYPE")==0) {
    ...
    h->num_angles=f;
  } else if(strcasecmp(field, "Z_ROTATION_ANGLES")==0) {
  ...
  } else if(strcasecmp(field, "GATE_DURATIONR")==0) {
    h->gate_duration=ii;
  } else if(strcasecmp(field, "R_WAVE_OFFSET")==0) {
    h->r_wave_offset=ii;
  } else if(strcasecmp(field, "NUM_ACCEPTED_BEATS")==0) {
