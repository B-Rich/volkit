/******************************************************************************
  ...
  File:        nifti.c
  Description: Procedures for reading and writing NIfTI-1 PET images.
  ...
  2013-03-18 Vesa Oikonen
       First created. Currently only read functions.
  2013-03-22 VO
       Added niftiWriteReader().
  2013-03-26 VO
       Added functions niftiCreateFNames() and niftiRemove().
       Bug fix in niftiExists() and niftiWriteHeader().
       niftiWriteHeader() verifies magic number before writing anything.
  2013-04-03 VO
       Fixed a serious bug in niftiReadImagedata().
...
/*****************************************************************************/
/** Remove any extensions from Nifti filename, leaving only base file name.
 */
void niftiRemoveFNameExtension(
  ...
  cptr=strrchr(fname, '.'); if(cptr==NULL) return;
  if(strcasecmp(cptr, ".img")==0 || strcasecmp(cptr, ".nii")==0)
    *cptr=(char)0;
}
/*****************************************************************************/

/*****************************************************************************/
/** Construct the filenames for NIfTI image.
...
int niftiCreateFNames(
  ...
  strcpy(basename, filename); niftiRemoveFNameExtension(basename);
  
  /* Create database filenames */
  if(fileformat==IMG_NIFTI_1D) {
    if(hdrfile!=NULL) sprintf(hdrfile, "%s.hdr", basename);
    if(imgfile!=NULL) sprintf(imgfile, "%s.img", basename);
  } else if(fileformat==IMG_NIFTI_1S) {
    if(hdrfile!=NULL) sprintf(hdrfile, "%s.nii", basename);
    if(imgfile!=NULL) sprintf(imgfile, "%s.nii", basename);
  } else
    return(2);
  if(siffile!=NULL) sprintf(siffile, "%s.sif", basename);
  return(0);
}
/*****************************************************************************/
 ...
 *  specified NIfTI database. SIF is not deleted in any case.
...
int niftiRemove(
  ...
  if(verbose>0) printf("niftiRemove(%s, %d, ...)\n", dbname, fileformat);
...
int niftiExists(
  ...
  if(access(temp, 0) == -1) {
    ...
    if(access(temp, 0) == -1) {
      ...
      if(access(temp, 0) == -1) {
        ...
        if(status!=NULL) {
          if(combined)
            strcpy(status, "combined Nifti file is accessible");
          else
            strcpy(status, "Nifti files are accessible");
        }
  ...
  if(status!=NULL) {
    if(combined)
      strcpy(status, "combined Nifti file and SIF are accessible");
    else
      strcpy(status, "Nifti files and SIF are accessible");
  }
  return(2);
}
/*****************************************************************************/
...
/*****************************************************************************/
/** Read Nifti header contents. Currently, does not read Nifti-1 header
 *  extension.
\return Returns 0, if successful, otherwise >0.
 */
int niftiReadHeader(
  ...
  /* Determine from dim[0] if Analyze file is big or little endian */
  ...
  if(!same_order) swawbip(buf+32, 4); memcpy(&dsr->h.extents, buf+32, 4);
  if(!same_order) swabip(buf+36, 2); memcpy(&dsr->h.session_error, buf+36, 2);
  memcpy(&dsr->h.regular, buf+38, 1);
  memcpy(&dsr->h.dim_info, buf+39, 1);

  /* dim */
  if(!same_order) swabip(buf+40, 16); memcpy(dsr->h.dim, buf+40, 16);
  /* intent parameters */
  if(!same_order) swawbip(buf+56, 4); memcpy(&dsr->h.intent_p1, buf+56, 4);
  if(!same_order) swawbip(buf+60, 4); memcpy(&dsr->h.intent_p2, buf+60, 4);
  if(!same_order) swawbip(buf+64, 4); memcpy(&dsr->h.intent_p3, buf+64, 4);
  if(!same_order) swabip(buf+68, 2); memcpy(&dsr->h.intent_code, buf+68, 2);

  /*  */
  if(!same_order) swabip(buf+70, 2); memcpy(&dsr->h.datatype, buf+70, 2);
  if(!same_order) swabip(buf+72, 2); memcpy(&dsr->h.bitpix, buf+72, 2);
  if(!same_order) swabip(buf+74, 2); memcpy(&dsr->h.slice_start, buf+74, 2);
  if(!same_order) swawbip(buf+76, 32); memcpy(dsr->h.pixdim, buf+76, 32);
  if(!same_order) swawbip(buf+108, 4); memcpy(&dsr->h.vox_offset, buf+108, 4);
  if(!same_order) swawbip(buf+112, 4); memcpy(&dsr->h.scl_slope, buf+112, 4);
  if(!same_order) swawbip(buf+116, 4); memcpy(&dsr->h.scl_inter, buf+116, 4);
  if(!same_order) swabip(buf+120, 2); memcpy(&dsr->h.slice_end, buf+120, 2);
  memcpy(&dsr->h.slice_code, buf+122, 1);
  memcpy(&dsr->h.xyzt_units, buf+123, 1);
  if(!same_order) swawbip(buf+124, 4); memcpy(&dsr->h.cal_max, buf+124, 4);
  if(!same_order) swawbip(buf+128, 4); memcpy(&dsr->h.cal_min, buf+128, 4);
  if(!same_order) swawbip(buf+132, 4); memcpy(&dsr->h.slice_duration,buf+132,4);
  if(!same_order) swawbip(buf+136, 4); memcpy(&dsr->h.toffset, buf+136, 4);
  if(!same_order) swawbip(buf+140, 4); memcpy(&dsr->h.glmax, buf+140, 4);
  if(!same_order) swawbip(buf+144, 4); memcpy(&dsr->h.glmin, buf+144, 4);
  ...
  if(!same_order) swabip(buf+252, 2); memcpy(&dsr->h.qform_code, buf+252, 2);
  if(!same_order) swabip(buf+254, 2); memcpy(&dsr->h.sform_code, buf+254, 2);
  if(!same_order) swawbip(buf+256, 4); memcpy(&dsr->h.quatern_b, buf+256, 4);
  if(!same_order) swawbip(buf+260, 4); memcpy(&dsr->h.quatern_c, buf+260, 4);
  if(!same_order) swawbip(buf+264, 4); memcpy(&dsr->h.quatern_d, buf+264, 4);
  if(!same_order) swawbip(buf+268, 4); memcpy(&dsr->h.qoffset_x, buf+268, 4);
  if(!same_order) swawbip(buf+272, 4); memcpy(&dsr->h.qoffset_y, buf+272, 4);
  if(!same_order) swawbip(buf+276, 4); memcpy(&dsr->h.qoffset_z, buf+276, 4);
  if(!same_order) swawbip(buf+280, 16); memcpy(dsr->h.srow_x, buf+280, 16);
  if(!same_order) swawbip(buf+296, 16); memcpy(dsr->h.srow_y, buf+296, 16);
  if(!same_order) swawbip(buf+312, 16); memcpy(dsr->h.srow_z, buf+312, 16);
...
int niftiReadImagedata(
  /** File pointer to start of image data file, opened previously in
   *  binary mode */
  FILE *fp,
  /** Pointer to previously filled Nifti header struct */
  NIFTI_DSR *dsr,
  /** Frame number to read [1..number of frames] */
  int frame,
  /** Pointer to image float data allocated previously for dimz*dimy*dimx
   *  floats */
  float *data,
  /** Verbose level; if zero, then nothing is printed to stderr or stdout */
  int verbose,
  ...
  if(verbose>0) printf("niftiReadImagedata(fp, h, %d, data, %d)\n",
                       frame, verbose);
  ...
  if(dimNr>3) dimt=dsr->h.dim[4]; if(frame>dimt) return(-1);
  ...
  if(ftell(fp)!=start_pos) {
    if(status!=NULL)
      sprintf(status, "could not move to start_pos %d", start_pos);
    free(mdata); return(7);
  }
  ...
  if((n=fread(mptr, rawSize, 1, fp)) < 1) {
    if(status!=NULL)
      sprintf(status, "could read only %d bytes when request was %d",
              n, rawSize);
    free(mdata); return(8);
  }
...
/*****************************************************************************/
/** Write NIfTI-1 header contents. Currently, does not write header
 *  extension.
...
int niftiWriteHeader(
  ...
  /* Make sure that buffers are all zeroes to beging with */
  ...
  if(fwrite(buf2, 1, NIFTI_HEADER_EXTENDER_SIZE, fp)
