/******************************************************************************
  ...
  ecat7p.c   (c) 2003-2007 by Turku PET Centre

  Procedures for printing ECAT 7.x (header) contents.


  Version:
  2003-07-17 Vesa Oikonen
  2004-05-23 VO
    Added ecat7datatype(short int data_type), and used too.
    Added some units etc.
  2004-08-15 VO
    Main header sex and dexterity are checked printed as space instead of
    (char)0.
  2004-09-20 VO
    Doxygen style comments are corrected.
  2007-03-21 VO
    strftime() call changed so that even Mingw can understand it.
  2007-17-07 Harri Merisaari
    fixed for ANSI
  2007-09-10 VO
    Return value of localtime() is checked.
  2008-07-24 VO
    Added ecat7PrintSubheader().
...
void ecat7PrintMainheader(ECAT7_mainheader *h, FILE *fp) {
  ...
  lt=(time_t)h->scan_start_time; st=localtime(&lt);
  if(st!=NULL) strftime(tmp, 32, "%Y-%m-%d %H:%M:%S", st); /* %T did not work with Mingw */
  ...
  for(i=0; i<15; i++) fprintf(fp, " %g", h->bed_position[i]);
  fprintf(fp, "\n");
  ...
  lt=(time_t)h->dose_start_time; st=localtime(&lt);
  if(st!=NULL) strftime(tmp, 32, "%Y-%m-%d %H:%M:%S", st); /* %T did not work with Mingw */
  else strcpy(tmp, "1900-01-01 00:00:00");
  ...
  fprintf(fp, "fill_cti :=");
  for(i=0; i<6; i++) fprintf(fp, " %d", h->fill_cti[i]);
  fprintf(fp, "\n");
}
/*****************************************************************************/
...
void ecat7PrintImageheader(ECAT7_imageheader *h, FILE *fp) {
  ...
  fprintf(fp, "fill_cti :=");
  for(i=0; i<87; i++) fprintf(fp, " %d", h->fill_cti[i]); fprintf(fp, "\n");
  fprintf(fp, "fill_user :=");
  for(i=0; i<49; i++) fprintf(fp, " %d", h->fill_user[i]); fprintf(fp, "\n");
}
/*****************************************************************************/
...
void ecat7PrintScanheader(ECAT7_scanheader *h, FILE *fp) {
  ...
  for(i=0; i<64; i++) fprintf(fp, " %d", h->num_z_elements[i]);
  fprintf(fp, "\n");
  ...
  for(i=0; i<128; i++) fprintf(fp, " %g", h->uncor_singles[i]);
  fprintf(fp, "\n");
}
/*****************************************************************************/
...
void ecat7PrintAttenheader(ECAT7_attenheader *h, FILE *fp) {
  ...
  for(i=0; i<8; i++) fprintf(fp, " %E", h->additional_atten_coeff[i]);
  fprintf(fp, "\n");
  ...
  for(mi=nr=0; mi<mlist.matrixNr; mi++) {
    ...
    switch(mh.file_type) {
      case ECAT7_ATTEN:
        ...
        if(ret==0) ecat7PrintAttenheader(&atten_header, stdout);
        break;
      case ECAT7_3DNORM:
        ret=ecat7ReadNormheader(fp, mlist.matdir[mi].strtblk, &norm_header);
        if(ret==0) ecat7PrintNormheader(&norm_header, stdout);
        break;
      case ECAT7_IMAGE8:
      case ECAT7_IMAGE16:
      case ECAT7_VOLUME8:
