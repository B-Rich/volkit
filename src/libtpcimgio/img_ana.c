/******************************************************************************
  ...
  Library:     img_ana.c
  Description: I/O routines for IMG data from/to Analyze 7.5 format.
  ...
  2007-01-31 Vesa Oikonen
    ...
    Prompts and randoms are read from SIF data to IMG when reading Analyze.
  2007-02-27 VO
    Bug correction in imgWriteAnalyze(): fp was not closed in all errors.
  2007-03-25 VO
    Added functions imgReadAnalyzeHeader(), imgGetAnalyzeHeader(),
    imgSetAnalyzeHeader(), imgReadAnalyzeFrame(), imgReadAnalyzeFirstFrame(),
    and imgWriteAnalyzeFrame().
  2007-17-07 Harri Merisaari
    Modified for optional ANSi compatibility    
  2007-09-10 VO
    Return value of localtime() is checked.
  2008-07-07 VO
    If information on decay correction is not found in Analyze header, then
    it is assumed that image data is corrected for decay; previously assumed
    that image was NOT corrected for decay.
  2009-12-10 VO
    strcpy() replaced with strncpy() in filling header fields.
  2011-08-17 VO
    imgSetAnalyzeHeader() writes exp_date in format YYYYMMDD instead of
    YYYY-MM-DD because the header field length is only 10 bytes.
  2012-02-28 VO
    imgReadAnalyze() uses anaExistsNew() instead of anaExists().
  2012-09-21 VO
    Applied defines IMG_DC_UNKNOWN, IMG_DC_CORRECTED, and IMG_DC_NONCORRECTED.
    imgReadAnalyzeFrame() sets decay correction factor to zero.
  2012-10-03 VO
    Added function img2sif() and sif2img().
    imgWriteAnalyze() writes also SIF when possible; frame-by-frame writing
    does not save SIF even now.
  2012-10-11 VO
    Fixed a typo in comments.
  2013-03-23 VO
    Removed possible memory leaks in imgWriteAnalyzeFrame().
  2013-04-02 VO
    Fixed the bug that study number was sometimes set to ".".
  2013-04-04 VO
    img2sif() and sif2img() are moved to img_sif.c.
    imgWriteAnalyze() creates count data in new SIF.
...
int imgGetAnalyzeHeader(IMG *img, ANALYZE_DSR *h) {
  ...
  /* Copy information from Analyze header */
  img->type=IMG_TYPE_IMAGE;
  strncpy(img->studyNr, h->hist.patient_id, MAX_STUDYNR_LEN);
  if(strcmp(img->studyNr, ".")==0) strcpy(img->studyNr, "");
