/******************************************************************************
  ...
  Library:     img_e63.c
  Description: ECAT 6.3 I/O routines for IMG data.
  ...
  2007-01-31 Vesa Oikonen
    Functions moved from imgfile.c.
    If valid study number is found in user_process_code, then take it from there.
    Patient_id and study_description are read and written.
    Prompts and randoms (delayed) are read and written.
  2007-03-25 VO
    Added functions imgGetEcat63MHeader(), imgSetEcat63MHeader(),
    imgEcat63Supported(), imgGetEcat63Fileformat(), imgReadEcat63Header(),
    imgReadEcat63FirstFrame(), imgReadEcat63Frame(), imgWriteEcat63Frame(),
    and imgSetEcat63SHeader().
  2007-09-10 VO
    Return value of localtime() is checked.
  2008-11-06 VO
    study_description is copied with strncpy(), not strcpy().
  2012-05-09 VO
    Removed unused variables.
  2012-09-21 VO
    imgReadEcat63Frame() sets img.decayCorrected on, when decay correction
    factor in header is >1 (like in imgReadEcat63).
    New defines for img.decayCorrected are applied.
    When ECAT image does not contain decay correction factors,
    img.decayCorrFactor[] is set to zero.
...
int ecat63ReadAllToImg(const char *fname, IMG *img) {
  ...
  img->scanStart=mktime(&scanStart); if(img->scanStart==-1) img->scanStart=0;
  ...
  strncpy(img->patientID, main_header.patient_id, 15);
  ...
  strncpy(img->studyDescription, main_header.study_description, 32);
  ...
  if(!img->studyNr[0] && studynr_validity_check(img->userProcessCode))
    strcpy(img->studyNr, img->userProcessCode);
  ...
  for(m=0; m<mlist.matrixNr; m++) if(mlist.matdir[m].matstat==1) {
    ...
    if(main_header.file_type==IMAGE_DATA) {
      ret=ecat63ReadImageheader(fp, mlist.matdir[m].strtblk, &image_header);
      if(dim_x!=image_header.dimension_1 || dim_y!=image_header.dimension_2)
        ret=1;
      scale=image_header.quant_scale;
      if(image_header.ecat_calibration_fctr>0.0 &&
         fabs(main_header.calibration_factor/image_header.ecat_calibration_fctr
              - 1.0)>0.0001)
 ...
 * @return 0 if ok, 1 invalid data, 2 image status is not 'oocupied', 
...
int ecat63WriteAllImg(const char *fname, IMG *img) {
  ...
  strncpy(main_header.user_process_code, img->userProcessCode, 10);
 ...
 * @param fname name of the input ECAT 6.3 file
 * @param img data structure in which the file is read
 * @return 0 if ok, 1 next plane was requested but not found anymore,
...
int ecat63ReadPlaneToImg(const char *fname, IMG *img) {
  ...
  //printf("img->scanStart=%d\n", img->scanStart);
  if(img->scanStart==-1) img->scanStart=0;
  img->axialFOV=10.*main_header.axial_fov;
  img->transaxialFOV=10.*main_header.transaxial_fov;
  strncpy(img->studyNr, main_header.study_name, MAX_STUDYNR_LEN);
  strncpy(img->patientName, main_header.patient_name, 31);
 ...
 *
 * @param fname name of the output ECAT 6.3 file
 * @param img data structure from which the data is written 
...
void imgGetEcat63MHeader(IMG *img, ECAT63_mainheader *h) {
  ...
  {
  memset(&scanStart, 0, sizeof(struct tm));
  scanStart.tm_year=h->scan_start_year-1900;
  scanStart.tm_mon=h->scan_start_month-1;
  scanStart.tm_mday=h->scan_start_day; scanStart.tm_yday=0;
  scanStart.tm_hour=h->scan_start_hour;
  scanStart.tm_min=h->scan_start_minute;
  scanStart.tm_sec=h->scan_start_second;
  scanStart.tm_isdst=-1;
  img->scanStart=mktime(&scanStart); if(img->scanStart==-1) img->scanStart=0;
  }
  img->axialFOV=10.0*h->axial_fov;
  img->transaxialFOV=10.0*h->transaxial_fov;
  strncpy(img->studyNr, h->study_name, MAX_STUDYNR_LEN);
  strncpy(img->patientName, h->patient_name, 31);
  strncpy(img->patientID, h->patient_id, 15);
  ...
  strncpy(img->studyDescription, h->study_description, 32);
...
void imgSetEcat63MHeader(IMG *img, ECAT63_mainheader *h) {
  ...
  scanStart=localtime(&img->scanStart);
  if(scanStart!=NULL) {
    h->scan_start_year=scanStart->tm_year+1900;
    h->scan_start_month=scanStart->tm_mon+1;
    h->scan_start_day=scanStart->tm_mday;
    h->scan_start_hour=scanStart->tm_hour;
    h->scan_start_minute=scanStart->tm_min;
    h->scan_start_second=scanStart->tm_sec;
  } else {
  ...
  strcpy(h->study_name, img->studyNr);
  strcpy(h->patient_name, img->patientName);
  strcpy(h->patient_id, img->patientID);
  strncpy(h->user_process_code, img->userProcessCode, 10);
  strncpy(h->study_description, img->studyDescription, 32);
}
/*****************************************************************************/
 ...
 *  filled.
 *  Please note: ECAT 6.3 files do not have a magic number, therefore, do
 ...
 * @param h target sub header stucture
 */
