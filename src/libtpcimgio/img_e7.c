/******************************************************************************
  ...
  Library:     img_e7.c
  Description: ECAT 7 I/O routines for IMG data.
  ...
  2007-02-27 Vesa Oikonen
    Functions moved from imgfile.c.
    Bug correction in imgWrite2DEcat7():
    prompts were set after the header was written.
  2007-03-25 VO
    Added functions imgReadEcat7Header(), imgEcat7Supported(),
    imgReadEcat7Frame(), imgReadEcat7FirstFrame(), imgGetEcat7Fileformat(),
    imgWriteEcat7Frame(), and imgSetEcat7SHeader().
  2007-04-03 VO
    Added support for ECAT 7 polar maps.
  2007-17-07 Harri Merisaari
    Modified timezone correction in for ANSI compatibility and for 
    other timezones in imgGetEcat7MHeader
  2007-09-07 VO
    The rest of strcpy's replaced by strncpy's in imgGetEcat7MHeader(),
    most importantly study_description, where overflow caused accidental 
    change of IMG type. 
  2007-09-10 VO
    Return value of localtime() is checked.
  2007-09-12 VO
    Bug correction in imgReadEcat7Frame(): did not set 2D image plane numbers.
  2010-02-12 VO
    Timezone correction added also into imgSetEcat7MHeader().
  2012-02-04 VO
    Added support for main header field branching_factor.
  2012-09-21 VO
    imgReadEcat7Frame() sets img.decayCorrected on, when decay correction
    factor in header is >1 (like in imgReadEcat7).
  ...
  2012-09-24 VO
    Bug fix in reading 3D sinogram frame times in imgReadEcat7Frame().
 ...
 * @param img initialized IMG structure
 *
...
void imgGetEcat7MHeader(IMG *img, ECAT7_mainheader *h) {
...
#ifndef __STRICT_ANSI__
  /* refresh timezone information (might be out of date) */
  tzset();
  /* correct scan start time in hours */
  lt = (time_t)img->scanStart;
  scanStartTime = localtime(&lt);
  if(scanStartTime!=NULL) {
    scanStartTime->tm_hour -= (timezone/3600);
    img->scanStart = mktime(scanStartTime);
  }
#endif
  ...
  strncpy(img->patientName, h->patient_name, 32);
...
void imgSetEcat7MHeader(IMG *img, ECAT7_mainheader *h) {
  ...
  tzset();
  /* correct scan start time in hours */
  lt = (time_t)h->scan_start_time;
  scanStartTime = localtime(&lt);
  if(scanStartTime!=NULL) {
    scanStartTime->tm_hour += (timezone/3600);
    h->scan_start_time = mktime(scanStartTime);
  }
#endif
