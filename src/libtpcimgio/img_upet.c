/*****************************************************************************
  ...
  Library:      img_upet.c
  Description:  I/O routines for IMG data from/to Siemens Inveon format.
  ...
  2012-02-04 Vesa Oikonen
    First started with contents from micropet.c in the same library.
    Branching fraction is copied to IMG struct, since a new field for it
    was added there.
  2012-02-20 VO
    Added functions imgReadMicropetHeader(), imgGetMicropetHeader(),
...
int imgReadMicropetHeader(
  ...
  ret=upetExists(dbname, hdrfile, NULL);
 ...
 *  data structure. MicroPET image consists of two files in the same 
 *  directory: fname.hdr and fname.img.
...
int imgReadMicropetFrame(
  ...
  ret=upetExists(fname, hdrfile, datfile);
  if(ret==0) {imgSetStatus(img, STATUS_NOFILE); return STATUS_NOFILE;}
