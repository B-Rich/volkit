/******************************************************************************
  ...
  ecat63r.c  (c) 2003,2004 by Turku PET Centre
  ...
  2003-09-05 VO
    Added function ecat63ReadImageMatrix() and ecat63ReadScanMatrix().
  2003-09-08 VO
    Added function ecat63pxlbytes().
  2004-06-21 VO
    Larger range allowed for calibration factor in ecat63ReadMainheader().
  2004-09-20 VO
...
int ecat63ReadScanMatrix(FILE *fp, int first_block, int last_block, ECAT63_scanheader *h, float **fdata) {
  ...
  *fdata=(float*)NULL;
  ...
  /* Read subheader */
  ret=ecat63ReadScanheader(fp, first_block, h);
  if(ret) {
    sprintf(ecat63errmsg, "cannot read subheader (%d).\n", ret);
    return(5);
  }
