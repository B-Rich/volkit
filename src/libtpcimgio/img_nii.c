/******************************************************************************
  ...
  File:        img_nii.c
  Description: NIfTI-1 PET image I/O routines for IMG data.
  ...
  2013-03-26 VO
       ...
       Added functions imgSetNiftiHeader() and imgWriteNiftiFrame().
  2013-03-28 VO
       New IMG struct fields are copied to/from NIfTI header.
  2013-04-02 VO
       Added function imgWriteNifti().
       imgReadNiftiFrame() sets IMG plane numbers.
  2013-04-04 VO
       imgWriteNifti() creates count data in new SIF.
...
int imgReadNifti(
  ...
  ret=imgReadNiftiHeader(filename, img, verbose); if(ret) {return(ret);}
  ...
  for(fi=0; fi<img->dimt; fi++) {
    ret=imgReadNiftiFrame(filename, 1+fi, img, fi, verbose);
    if(ret) return(ret);
  }
...
int imgReadNiftiFirstFrame(
  ...
  ret=imgReadNiftiFrame(filename, 1, img, 0, verbose); if(ret) return(ret); 
...
int imgReadNiftiHeader(
  ...
  ret=niftiExists(basefile, hdrfile, datfile, siffile, &dsr, verbose, tmp);
  if(ret==0) {
  ...
  ret=imgGetNiftiHeader(img, &dsr, verbose);
  if(ret!=0) {
...
int imgReadNiftiFrame(
  ...
  fptr=fdata;
  ret=niftiReadImagedata(fp, &dsr, frame_to_read, fptr, verbose, tmp);
...
int imgSetNiftiHeader(
  ...
  if(img==NULL || (img->status!=IMG_STATUS_INITIALIZED &&
                   img->status!=IMG_STATUS_OCCUPIED)) {
    if(verbose>0) fprintf(stderr, "Error: invalid IMG argument\n");
    return(STATUS_FAULT);
  }
  if(dsr==NULL) {
  ...
  dsr->h.extents=16384; // not used in NIfTI
  dsr->h.regular='r'; // not used in NIfTI
  ...
  dsr->h.slice_end=0;
  dsr->h.slice_code=0;
  dsr->h.xyzt_units=NIFTI_UNITS_SEC+NIFTI_UNITS_MM;
  dsr->h.cal_max=fmax;
  dsr->h.cal_min=0.0;
  ...
  strncpy(dsr->h.descrip, img->studyNr, 80);
...
int imgWriteNiftiFrame(
   ...
   */
  imgInit(&test_img);
  fileis=niftiExists(dbname, hdrfile, imgfile, siffile, &dsr, verbose-1, NULL);
  if(fileis==0) { // not existing
    if(verbose>1) printf("  writing 1st frame to a new file\n");
  ...
  fdata=(float*)malloc(voxNr*sizeof(float));
  if(fdata==NULL) {
